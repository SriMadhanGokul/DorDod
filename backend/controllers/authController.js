...
const setPassword = async (req, res) => {
  try {
    ...
    sendTokenResponse(
      ...
      201,
      res,
      "Account created successfully! Welcome to SkillSpark 🚀",
    );
  } catch (error) {
  ...
  try {
    ...
    if (!record)
      return res
        ...
        .json({
          success: false,
          message: "No OTP request found. Please start registration again.",
        });
  ...
  try {
    ...
    if (user.isGoogleUser && !user.hasPassword)
      return res.status(400).json({
        success: false,
        message:
          "This account uses Google Sign-In. Please set a password first or use Google login.",
        needsPassword: true,
      });
...
const googleCallback = (req, res) => {
  try {
    ...
    if (!user.hasPassword) {
      return res.redirect(
        `${process.env.CLIENT_URL}/setup-password?uid=${user._id}`,
      );
    }
...
const setupPassword = async (req, res) => {
  try {
    ...
    res
      ...
      .json({
        success: true,
        message: "Password set successfully! You can now login with email too.",
      });
  } catch (error) {
