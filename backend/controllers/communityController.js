...
const formatPost = (post, currentUserId) => {
  ...
  return {
    ...
    isOwner:       post.user?._id?.toString() === uid,
    content:       post.content,
    tags:          post.tags,
    likes:         post.likes?.length || 0,
    liked:         post.likes?.some(id => id.toString() === uid) || false,
    comments:      (post.comments || []).map(c => ({
      ...
      isOwner:     c.user?._id?.toString() === uid,
      isPostOwner: post.user?._id?.toString() === uid,
...
const createPost = async (req, res) => {
  try {
    ...
    let finalMediaType = 'none', finalMediaUrl = '', mediaFileName = '';

    if (req.file) {
      const mime = req.file.mimetype;
      finalMediaType = mime.startsWith('image/') ? 'image' : mime.startsWith('video/') ? 'video' : 'file';
      const { url } = await uploadToFirebase(
        req.file.buffer, req.file.originalname, req.file.mimetype,
        `community/${req.user.id}`
      );
      finalMediaUrl  = url;
    ...
    const tagList = tags ? (Array.isArray(tags) ? tags : [tags]) : ['General'];
    const post    = await Post.create({
      user: req.user.id, content: content?.trim() || '', tags: tagList,
      mediaType: finalMediaType, mediaUrl: finalMediaUrl, mediaFileName,
...
const deletePost = async (req, res) => {
  try {
    ...
    if (!post) return res.status(404).json({ success: false, message: 'Post not found or not yours' });
    if (post.mediaUrl?.includes('storage.googleapis.com')) {
      await deleteFromFirebase(post.mediaUrl).catch(() => {});
    }
...
const toggleLike = async (req, res) => {
  try {
    const post = await Post.findById(req.params.id).populate('user','name avatar').populate('comments.user','name avatar');
...
const addComment = async (req, res) => {
  try {
    ...
    res.status(201).json({ success: true, data: {
      _id: newC._id, content: newC.content,
      author: newC.user?.name || 'You', avatar: newC.user?.avatar || '',
      userId: req.user.id, isOwner: true,
      isPostOwner: post.user?._id?.toString() === req.user.id?.toString(),
      edited: false, time: newC.createdAt,
    }});
...
const editComment = async (req, res) => {
  try {
    const { content } = req.body;
    if (!content?.trim()) return res.status(400).json({ success: false, message: 'Comment cannot be empty' });
    const post = await Post.findById(req.params.postId).populate('user','name avatar').populate('comments.user','name avatar');
    if (!post) return res.status(404).json({ success: false, message: 'Post not found' });
    const comment = post.comments.id(req.params.commentId);
    if (!comment) return res.status(404).json({ success: false, message: 'Comment not found' });
    if (comment.user._id.toString() !== req.user.id.toString())
      return res.status(403).json({ success: false, message: 'Not authorized' });
    comment.content = content.trim(); comment.edited = true;
    post.markModified('comments');
    await post.save();
    res.status(200).json({ success: true, data: {
      _id: comment._id, content: comment.content, author: comment.user?.name || 'You',
      userId: req.user.id, isOwner: true, edited: true, time: comment.createdAt,
    }});
  } catch (err) { res.status(500).json({ success: false, message: 'Failed to edit comment' }); }
};
