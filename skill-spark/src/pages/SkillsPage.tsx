...
const EMPTY_CUSTOM = {
  ...
  category: "Technical",
  status: "current",
  currentLevel: "Beginner (Awareness Level)",
};
...
export default function SkillsPage() {
  ...
  const openEditCustom = (s: CustomSkill) => {
    ...
    setEditForm({
      ...
      alreadyKnows: (s.alreadyKnows || [])
        .map((t: any) => (typeof t === "string" ? t : t?.name || ""))
        .filter(Boolean),
      wantsToLearn: (s.wantsToLearn || [])
        .map((t: any) => (typeof t === "string" ? t : t?.name || ""))
        .filter(Boolean),
  ...
  const CustomFormFields = ({ form, setForm }: { form: any; setForm: any }) => (
    <div className="space-y-4">
      <div>
        ...
        <input
          placeholder="e.g. Video Editing, Public Speaking"
          value={form.skillName}
          onChange={(e) =>
            setForm((p: any) => ({ ...p, skillName: e.target.value }))
          }
      ...
      <div>
        ...
        <select
          ...
          onChange={(e) =>
            setForm((p: any) => ({ ...p, currentLevel: e.target.value }))
          }
          className="input-field mt-1"
        >
          {SKILL_LEVELS.map((l) => (
      ...
      <div>
        <label className="text-xs font-medium text-foreground-muted flex items-center gap-1">
          <FaTag className="w-3 h-3" /> Already Know{" "}
          <span className="font-normal opacity-60">(Enter to add)</span>
        </label>
        <TagInput
          tags={form.alreadyKnows}
          onChange={(tags) =>
            setForm((p: any) => ({ ...p, alreadyKnows: tags }))
          }
      ...
      <div>
        <label className="text-xs font-medium text-foreground-muted flex items-center gap-1">
          <FaTag className="w-3 h-3" /> Want to Learn{" "}
          <span className="font-normal opacity-60">(Enter to add)</span>
        </label>
        <TagInput
          tags={form.wantsToLearn}
          onChange={(tags) =>
            setForm((p: any) => ({ ...p, wantsToLearn: tags }))
          }
  ...
  // ✅ REMOVED DashboardLayout from loading state
  if (loading)
    return (
  ...
  // ✅ REMOVED DashboardLayout wrapper from return statement
  return (
        ...
        <>
          <div>
            ...
            <p className="text-foreground-muted mt-1">
              Select the path — we'll track all your skills
            </p>
          </div>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            {careers.map((career) => (
              ...
              >
                ...
                <h3 className="font-bold text-base group-hover:text-primary">
                  {career.title}
                </h3>
            ...
            {Object.entries(grouped).map(([category, skills]) => {
              ...
              return (
                <div key={category} className="card-elevated">
                  <button
                    onClick={() =>
                      setExpandedCat(expandedCat === category ? null : category)
                    }
                        ...
                        return (
                          ...
                          >
                            <div className="flex items-center gap-3 flex-1 min-w-0">
                              ...
                              {skill.addedToGoal && (
                                <span className="text-xs bg-secondary/20 text-secondary-foreground px-2 py-0.5 rounded-full shrink-0">
                                  ✓ Goal
                                </span>
                              )}
                            </div>
                            <div className="flex items-center gap-2 shrink-0 ml-2">
                              <select
                                value={skill.status}
                                onChange={(e) =>
                                  handleStatusChange(skill._id, e.target.value)
                                }
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {learnTags.map((tag, i) => (
                            <div
                              key={tag._id || i}
                              className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl border transition-all ${tag.addedToGoal ? "bg-success/15 border-success/40 text-success" : "bg-primary/10 border-primary/30 text-primary"}`}
                            >
      ...
      {editSkill && (
        <div className="fixed inset-0 bg-foreground/50 flex items-start justify-center z-50 p-4 pt-8 overflow-y-auto">
          <div className="bg-card rounded-2xl p-6 w-full max-w-md my-auto animate-fade-in">
            <div className="flex justify-between items-center mb-5">
              <h2 className="text-lg font-bold flex items-center gap-2">
                <FaEdit className="text-primary" /> Edit Skill
              </h2>
